...
![ArgoCD](https://img.shields.io/badge/Argo_CD-v3.3.4-EF7B4D?style=for-the-badge&logo=argo)
![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-ARC_v0.14.0-2088FF?style=for-the-badge&logo=githubactions)
![Sealed Secrets](https://img.shields.io/badge/Sealed_Secrets-v2.18.4-1b1b1b?style=for-the-badge)
![Longhorn](https://img.shields.io/badge/Longhorn-v1.11.1-5F224B?style=for-the-badge&logo=longhornio)
![Velero](https://img.shields.io/badge/Velero-v1.18.0-009FDA?style=for-the-badge)
...
![Vault](https://img.shields.io/badge/HashiCorp_Vault-v1.19-000000?style=for-the-badge&logo=vault)
![ESO](https://img.shields.io/badge/External_Secrets-v0.14.3-FF5733?style=for-the-badge)
![Trivy](https://img.shields.io/badge/Trivy_Operator-v0.32.1-1904DA?style=for-the-badge)
![Status](https://img.shields.io/badge/Status-Blocks_A--K_Complete-brightgreen?style=for-the-badge)
...
| 🖥️ **2 bare-metal hosts** | ☸️ **4-node Kubernetes** | 🚀 **20 apps via GitOps** | 💾 **~3 min DR restore** | 📊 **Full observability** | 📚 **36 lessons documented** |
|:-:|:-:|:-:|:-:|:-:|:-:|
...
- **Kubernetes from scratch**: cluster bootstrapped with `kubeadm` (not managed EKS/GKE), 4 nodes (1 control-plane + 3 workers), containerd runtime, Calico CNI, MetalLB L2 — deep understanding of Kubernetes internals
- **GitOps App-of-Apps**: 20 applications synced via Argo CD — what is in Git is what is in the cluster. Pattern used by Netflix, Uber, Spotify
...
- **Secrets Management**: Sealed Secrets v2.18.4 + **HashiCorp Vault** (standalone, Raft) + **External Secrets Operator** — centralized secrets store with auto-sync to K8s Secrets
- **Security Scanning**: **Trivy Operator** — automated CVE scanning of containers, IaC misconfigs, exposed secrets. Results in Prometheus + Grafana dashboard
...
graph TB
    ...
    subgraph Platform["⚙️ Platform Layer"]
        ...
        NGINX["Ingress-NGINX v1.15.1"]
        CERT["cert-manager v1.20.0\nInternal CA"]
...
<table>
  <tr>
    <td align="center" width="50%">
      <b>Argo CD — 20 Apps Synced / Healthy</b><br/>
...
| **TLS** | cert-manager + internal CA | v1.20.0 | ✅ |
...
| **CMS (WordPress)** | WordPress + MariaDB (Bitnami Helm 29.2.7) | 6.8.2 + 11.8.3 | ✅ |
...
| **Backup/DR** | Velero + velero-plugin-for-aws | 1.18.0 + v1.13.1 | ✅ |
| **CI/CD** | GitHub Actions + ARC | v0.14.0 | ✅ |
| **Secrets (Git)** | Sealed Secrets | v2.18.4 | ✅ |
| **Secrets (Vault)** | HashiCorp Vault (standalone, Raft) | v1.19 | ✅ |
| **Secret Sync** | External Secrets Operator (ESO) | v0.14.3 | ✅ |
| **Security Scanning** | Trivy Operator (CVE, IaC, secrets) | v0.32.1 | ✅ |
...
| k8s-worker-01 | 10.44.81.111 | 111 | pve01 | **zone-a** | worker | 4 | 6 GB | 60 GB |
| k8s-worker-02 | 10.44.81.112 | 112 | pve02 | **zone-b** | worker | 4 | 4 GB | 60 GB |
| k8s-worker-03 | 10.44.81.113 | 113 | pve02 | **zone-b** | worker | 4 | 8 GB | 60 GB |
...
| ingress-nginx | ingress-nginx | v1.15.1, EXTERNAL-IP: 10.44.81.200 | ✅ |
| cert-manager | cert-manager | v1.20.0, internal CA (lab-ca-issuer) | ✅ |
| Longhorn | longhorn-system | v1.11.1, SC: `longhorn` + `longhorn-single` | ✅ |
...
| Argo CD | argocd | v3.3.4, **20 apps** Synced/Healthy | ✅ |
| Sealed Secrets | kube-system | v2.18.4, sealed-secrets-controller | ✅ |
| HashiCorp Vault | vault | v1.19, standalone+Raft, Longhorn 10Gi, Vault Agent Injector | ✅ |
| External Secrets Operator | external-secrets | v0.14.3, ClusterSecretStore `vault-backend` | ✅ |
| Trivy Operator | trivy-system | v0.32.1, ServiceMonitor enabled, Grafana Dashboard | ✅ |
...
| Velero | velero | v1.18.0, BSL Available, `--features=EnableCSI` | ✅ |
...
- **Trivy Operator v0.32.1** — automated CVE scanning of all container images, config audit, exposed secrets detection. ServiceMonitor → Prometheus → Grafana dashboard
- **HashiCorp Vault v1.19** (standalone + Raft, Longhorn PVC) — centralized secrets store with Web UI at `vault.lab.local`. Vault Agent Injector for auto-inject into pods
- **External Secrets Operator v0.14.3** — syncs secrets from Vault → K8s Secrets automatically via ClusterSecretStore
- Sealed Secrets v2.18.4 — encrypted secrets safe to commit to Git, decrypted only inside cluster
- cert-manager v1.20.0 with internal CA (`lab-ca-issuer`) — TLS on all services, no browser warnings
...
- 2-zone topology: zone-a (pve01: master-01 + worker-01), zone-b (pve02: worker-02 + worker-03)
...
- `terraform apply` — 4 VMs in one command (master + 3 workers)

### Block C — Configuration (Ansible)
- Playbook `kubeadm-cluster.yml`: swap off, sysctl, containerd, kubeadm
- `kubeadm init` + Calico CNI + join workers → **failed=0, all 4 nodes Ready**
...
- ✅ **Promtail v3.0.0** — DaemonSet 4/4 Running (all 4 nodes)
...
- ✅ **20 Applications**, all `Synced / Healthy`
...
├── ansible/
│   ├── inventory.ini               # 4-node cluster inventory
...
│   ├── apps/                       # Argo CD Application CRDs (20 apps)
...
│   │   └── dashboard-trivy.yaml           # Trivy Operator — CVE vulnerabilities by namespace/image
│   └── monitoring/
...
│   └── docs/                       # Wiki content (36 lessons, services, infra)
├── docs/
...
```bash
# Argo CD will automatically sync all 20 apps from Git
...
*Last updated: 10.04.2026 — 4-node cluster (worker-03 on pve02/zone-b added) ✅ | worker-01 resized to 4CPU/6GB ✅ | 36 Lessons Learned in Wiki | Velero BSL Available ✅ | Blocks A–K closed ✅*
